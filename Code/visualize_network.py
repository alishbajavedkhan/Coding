...
class App:
    ...
    def __init__(self, root, network, networkParams):
        ...
        # Start threads for network operation and UI updates
        threading.Thread(target=self.network.run, daemon=True).start()
        threading.Thread(target=self.displayCurrentRoutes, daemon=True).start()
        threading.Thread(target=self.displayCurrentDebug, daemon=True).start()
    ...
    def drawLine(self, addr1, addr2, c12, c21):
        ...
        label = self.canvas.create_text(tx, ty, text=t, fill=self.networkParams["visualize"]["lineFontColor"],
                                        state=NORMAL, font=tkinter.font.Font(size=self.networkParams["visualize"]["lineFontSize"], weight="bold"))
        return line, label
    ...
    def packetSend(self, packet, src, dst, latency):
        ...
        if self.clientFollowing:
            if packet.dstAddr == self.clientFollowing and packet.is_traceroute():
                fillColor = "green"
            else:
                return
        else:
            fillColor = "gray" if packet.is_traceroute() else "turquoise"
        ...
        threading.Thread(target=self.movePacket, args=(packetRect, velocityx, velocityy, numSteps, stepTime), daemon=True).start()
    ...
    def displayCurrentDebug(self):
        ...
        while True:
            if self.routerFollowing:
                debugText = self.network.routers[self.routerFollowing].debug_string()
...
def main():
    """Main function parses command line arguments and runs the network visualizer"""
    if len(sys.argv) < 2:
